"""
Gerenciador Manual de Imagens — ENEM 2009–2024

Estrutura de imagens no JSON:
  "imagens": [
    {"path": "2023/dia1/q012_1.jpg", "posicao": "antes_1"},
    {"path": "2023/dia1/q012_2.jpg", "posicao": "entre_2_3"}
  ]

Posições disponíveis (geradas dinamicamente pelo nº de parágrafos):
  "antes_1"      → antes do parágrafo 1
  "entre_1_2"    → entre §1 e §2
  "entre_2_3"    → entre §2 e §3
  ...
  "apos_ultimo"  → após o último parágrafo

Imagens de alternativas continuam em "imagens_alternativas":
  {"A": "2023/dia1/q012_alt_a.jpg"}
"""
...
# ── Helpers de dados ──────────────────────────────────────────────────────────
def carregar_json(ano: int) -> list:
...
def salvar_json(ano: int, questoes: list):
    jp = PASTA_JSON / f"enem_{ano}.json"
    with open(jp, "w", encoding="utf-8") as f:
        json.dump(questoes, f, ensure_ascii=False, indent=2)


def salvar_questao(ano: int, dia: str, num: int, q: dict):
    qs = carregar_json(ano)
    for i, aq in enumerate(qs):
        if aq["numero"] == num and aq.get("dia") == dia:
            qs[i] = q
            break
    salvar_json(ano, qs)
...
class JanelaFormula(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        tk.Button(row_btn, text="➕  Inserir fórmula",
                  bg="#7c6af7", fg="#fff", relief="flat",
                  font=("Segoe UI", 10, "bold"),
                  pady=7, cursor="hand2",
                  activebackground="#9580ff",
...
class JanelaVisualizacao(tk.Toplevel):
    ...
    def __init__(self, parent, path: Path):
        ...
        wrap = tk.Frame(self, bg="#1e1e2e")
        wrap.pack(fill="both", expand=True, padx=8, pady=(0, 8))
        canvas = tk.Canvas(wrap, bg="#1e1e2e", highlightthickness=0)
...
class JanelaRecorte(tk.Toplevel):
    ...
    def _build_ui(self):
        ...
        self._canvas = tk.Canvas(wrap, bg="#2a2a3e",
                                  cursor="crosshair",
                                  highlightthickness=1,
                                  highlightbackground="#7c6af7")
    ...
    def _on_drag(self, event):
        ...
        self._rect_id = self._canvas.create_rectangle(
            ...
            outline="#7c6af7", width=2, dash=(6, 3))
        s = self._scale
...
class App(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Gerenciador de Imagens — ENEM")
        ...
        self._thumbs: list             = []
        self._posicao_map: dict        = {}   # label → valor interno
        self._ultimo_campo             = None  # último widget editável com foco
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="  GERENCIADOR DE IMAGENS — ENEM",
        ...
        # Ano ◀ dropdown ▶
        row_ano = tk.Frame(left, bg=C.CARD)
        row_ano.pack(fill="x", padx=10, pady=2)
        tk.Button(row_ano, text="◀", bg=C.ENTRY_BG, fg=C.FG, relief="flat",
                  width=2, cursor="hand2",
                  command=self._ano_anterior).pack(side="left")
        tk.Label(row_ano, text="Ano", bg=C.CARD, fg=C.FG2,
                 font=("Segoe UI", 9), width=3).pack(side="left", padx=(4, 2))
        self.cb_ano = ttk.Combobox(row_ano, values=[str(a) for a in ANOS],
                                   width=6, state="readonly")
        self.cb_ano.set("2023")
        self.cb_ano.pack(side="left")
        self.cb_ano.bind("<<ComboboxSelected>>", lambda e: self._load_ano_dia())
        tk.Button(row_ano, text="▶", bg=C.ENTRY_BG, fg=C.FG, relief="flat",
                  width=2, cursor="hand2",
                  command=self._proximo_ano).pack(side="left", padx=(4, 0))

        # Dia
        row_dia = tk.Frame(left, bg=C.CARD)
        row_dia.pack(fill="x", padx=10, pady=2)
        tk.Label(row_dia, text="Dia", bg=C.CARD, fg=C.FG2,
                 font=("Segoe UI", 9), width=4, anchor="w").pack(side="left")
        self.cb_dia = ttk.Combobox(row_dia, values=DIAS, width=8, state="readonly")
        self.cb_dia.set("dia1")
        self.cb_dia.pack(side="left")
        self.cb_dia.bind("<<ComboboxSelected>>", lambda e: self._load_ano_dia())
    ...
    def _atualizar_questao(self):
        ...
        self.lbl_q_title.config(
            text=f"Questão {q['numero']:03d}  —  "
                 f"{self.cb_ano.get()} / {self.cb_dia.get()}")
    ...
    def _render_thumb_card(self, rel: str, label: str,
        ...
        if fp.exists():
            ...
            tk.Button(row_btns, text="👁 Ver",
                      bg="#313244", fg=self.FG, relief="flat",
            ...
            tk.Button(row_btns, text="✂ Recortar",
                      bg="#313244", fg=self.WARN, relief="flat",
                      font=("Segoe UI", 7), padx=4, pady=2, cursor="hand2",
            ...
            tk.Button(row_btns, text="🗑 Deletar",
                      bg="#313244", fg=self.DANGER, relief="flat",
                      font=("Segoe UI", 7), padx=4, pady=2, cursor="hand2",
            ...
            if posicao is not None and not label.startswith("alt_"):
                ...
                tk.Frame(card, bg="#45475a", height=1).pack(
    ...
    def _ir_para_numero(self):
        ...
        messagebox.showwarning("Não encontrada",
                               f"Q{num:03d} não encontrada neste dia.")

    def _ano_anterior(self):
        idx = ANOS.index(int(self.cb_ano.get()))
        if idx > 0:
            self.cb_ano.set(str(ANOS[idx - 1]))
            self._load_ano_dia()

    def _proximo_ano(self):
        idx = ANOS.index(int(self.cb_ano.get()))
        if idx < len(ANOS) - 1:
            self.cb_ano.set(str(ANOS[idx + 1]))
            self._load_ano_dia()
    ...
    def _adicionar_imagem(self):
        ...
        pasta_dest = PASTA_IMG / str(ano) / dia
        ...
        if tipo == "enunciado":
            ...
            if idx_existente is not None:
                # Apagar o arquivo antigo e reutilizar o mesmo nome
                ...
                # Nova imagem → próximo índice disponível
    ...
    def _deletar_imagem(self, rel: str):
        """Remove do JSON e apaga o arquivo do disco."""
        ...
        self.lbl_status.config(text="🗑 Imagem deletada do disco e do JSON.")
        self._atualizar_imgs_salvas(q)
    ...
    def _bind_teclado(self):
        ...
        # Ctrl+S e Ctrl+M no nível da janela (funciona quando nenhum widget tem foco)
        for seq in ("<Control-s>", "<Control-S>"):
            self.bind(seq, lambda e: self._salvar_tudo() or "break")
        # Ctrl+M = ASCII 0x0D = Return no Windows; bind ambos
    ...
    def _configurar_tab(self):
        ...
        # Campos que aceitam texto e devem ser rastreados como destino de fórmula
        self._campos_texto = [
            self.txt_enun,
            self.ent_cmd,
        ...
        for i, widget in enumerate(self._tab_order):
            ...
            # Ctrl+S em todos os widgets (Text e Entry) — captura antes da classe
            for seq in ("<Control-s>", "<Control-S>"):
                widget.bind(seq, lambda e: self._salvar_tudo() or "break")
            # Ctrl+M: bind todas as variantes porque Ctrl+M = Return no Windows
            for seq in ("<Control-m>", "<Control-M>", "<Control-Return>"):
                widget.bind(seq, lambda e: self._abrir_formula() or "break")

        # FocusIn em todos os campos de texto: rastreia o último campo ativo
        for w in self._campos_texto:
            w.bind("<FocusIn>", lambda e, campo=w: self._registrar_campo(campo), add="+")

        # Camada extra: bind_class captura widgets fora de _tab_order
    ...
    def _abrir_formula(self):
        ...
        try:
            ...
            # Prioridade: foco atual (se for campo válido) → último registrado → enunciado
            w = self.focus_get()
            if w not in campos:
            ...
            # Captura a posição do cursor AGORA, antes de a janela roubar o foco
    ...
    def _salvar_tudo(self):
        try:
            ...
            self.lbl_status.config(text="✓ Enunciado + alternativas salvos!")
        except Exception as exc:
    ...
    def _aplicar_formato(self, tipo: str):
        ...
        • Com texto selecionado:
            - Todos os chars têm Unicode → converte para Unicode (ex.: ₀₁₂ / ⁰¹²)
            - Algum char sem Unicode  → converte toda a seleção para LaTeX inline
              ex.: "dia" → $_{dia}$   |   "c" → $_{c}$   |   "2" → ₂
        • Sem seleção → insere placeholder LaTeX e posiciona cursor dentro.
        """
        w = self.focus_get()
        ...
        try:
            ...
            # Verifica se TODOS os chars não-espaço têm equivalente Unicode
            todos_unicode = all(
            ...
            if todos_unicode:
                ...
                # Fallback LaTeX inline — KaTeX renderiza corretamente no frontend
            ...
            # Sem seleção — insere placeholder LaTeX com cursor posicionado dentro
            placeholder = f"${latex_cmd}{{}}$"
            pos = w.index("insert")
            w.insert(pos, placeholder)
            # Posiciona cursor entre as chaves: $_{|}$
    ...
    def _prova_anterior(self):
        """... → dia2 do ano anterior → dia1 → dia2"""
        ano = int(self.cb_ano.get())
        dia = self.cb_dia.get()
        if dia == "dia2":
            self.cb_dia.set("dia1")
...
if __name__ == "__main__":
    ...
    _p = _ap.ArgumentParser(description="Gerenciador de Imagens ENEM")
    _p.add_argument("--revisao", action="store_true",
                    help="Modo revisao: navega apenas questoes do relatorio_erros.json com status=pendente")
    _args, _ = _p.parse_known_args()

    app = App()

    if _args.revisao:
        # Carrega relatorio_erros.json e constroi lista de (ano, dia, numero)
        _rel_path = Path(r"C:\PROJETOS\HENRYJR\relatorio_erros.json")
        if not _rel_path.exists():
            import tkinter.messagebox as _mb
            _mb.showwarning("Revisao", f"Arquivo nao encontrado:\n{_rel_path}\n\nAbrindo modo normal.")
        else:
            with open(_rel_path, encoding="utf-8") as _f:
                _erros = json.load(_f)
            _pendentes = [e for e in _erros if e.get("status") == "pendente"]
            if not _pendentes:
                import tkinter.messagebox as _mb
                _mb.showinfo("Revisao", "Nenhum erro pendente no relatorio_erros.json!\nAbrindo modo normal.")
            else:
                # Exibir banner de modo revisao no titulo
                app.title(f"Gerenciador — MODO REVISAO ({len(_pendentes)} pendentes)")
                # Navegar para o primeiro pendente
                _p0 = _pendentes[0]
                _ano0 = _p0.get("ano")
                _dia0 = _p0.get("dia", "dia1")
                _num0 = _p0.get("numero")
                if _ano0:
                    # Definir ano/dia via variaveis de controle do App, se existirem
                    try:
                        app._var_ano.set(str(_ano0))
                        app._var_dia.set(_dia0)
                        app._load_ano_dia()
                        # Navegar para o numero especifico
                        for _i, _q in enumerate(app._questoes):
                            if _q.get("numero") == _num0:
                                app._q_idx = _i
                                app._mostrar()
                                break
                    except Exception as _e:
                        pass  # nao fatal — abre no padrao
